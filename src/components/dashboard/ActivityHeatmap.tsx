...
const ActivityHeatmap: React.FC<ActivityHeatmapProps> = ({ data, title = 'Activity' }) => {
        ...
        <div className="overflow-x-auto">
          <div className="min-w-[750px]">
            ...
            <div className="flex mb-2 text-xs text-muted-foreground ml-8">
              {months.map((month, i) => (
                <span key={month} className="flex-1 first:pl-0">{month}</span>
              ))}
            </div>
                      ...
                      <Tooltip key={`${weekIndex}-${dayIndex}`}>
                        <TooltipTrigger asChild>
                          <div
                            className={cn(
                              ...
                              day.count === -1 ? 'invisible' : getIntensityClass(day.count),
                        ...
                        {day.count >= 0 && (
                          <TooltipContent side="top" className="text-xs">
                            <p>{day.count} problems on {formatDate(day.date)}</p>
                          </TooltipContent>
                        )}
                      </Tooltip>
                    ))}
